...
contract("CommitReveal", async (accounts) => {
  ...
  const expectedVotesForChoice1 = commitInfos.filter(commitInfo => commitInfo.vote === "1").length;
  const expectedVotesForChoice2 = commitInfos.filter(commitInfo => commitInfo.vote === "2").length;
  const expectedWinner = expectedVotesForChoice1 > expectedVotesForChoice2 ? "YES" : (
    expectedVotesForChoice1 < expectedVotesForChoice2 ? "NO" : "It was a tie!"
  );

  it("deploy contract", async() => {
    commitRevealWrapper = await CommitReveal.deployed();

    assert.notEqual(commitRevealWrapper.address, undefined, 'CommitReveal is not deployed');
  });

  it("users vote during phase period", async () => {
    await Promise.all(commitInfos.map(commitInfo => (
    ...
    const numberOfVotesCast = (await commitRevealWrapper.numberOfVotesCast()).toNumber();
    assert.equal(numberOfVotesCast, expectedNumberOfVotesCast, 'incorrect numberOfVotesCast');
  });
  ...
  it("users reveal their votes", async () => {
    ...
    assert.equal(votesForChoice1, expectedVotesForChoice1, 'incorrect votesForChoice1');
    assert.equal(votesForChoice2, expectedVotesForChoice2, 'incorrect votesForChoice2');
  });
